...
# ============================================================
# 向后兼容：为旧 app/ 代码保留的过渡函数
# ============================================================

def get_china_stock_info_unified(*args, **kwargs):
    """统一获取中国股票信息（兼容旧代码的过渡函数）

    等价于：route_to_vendor("get_stock_info", symbol, vendor="a_stock")
    """
    symbol = kwargs.get("symbol") or kwargs.get("code") or (
        args[0] if args else None
    )
    if symbol is None:
        return None
    try:
        return route_to_vendor("get_stock_info", symbol, vendor="a_stock")
    except Exception:
        return None